...
describe('GroupedWriter', () => {
  ...
  describe('A GroupedWriter instance', () => {
    it('should serialize a single triple', () => {
      const groupedWriter = new GroupedWriter();
      expect(
        groupedWriter.quadToString(new NamedNode('a'), new NamedNode('b'), new NamedNode('c')),
      ).toBe('<a> <b> <c> .\n');
    });
    ...
    it('should serialize a single quad', () => {
      const groupedWriter = new GroupedWriter();
      expect(
        groupedWriter.quadToString(new NamedNode('a'), new NamedNode('b'), new NamedNode('c'), new NamedNode('g')),
      ).toBe('<a> <b> <c> <g> .\n');
    });
    ...
    it('should serialize an array of triples', () => {
      ...
      expect(GroupedWriter.quadsToString(triples)).toBe('<a> <b> <c> .\n<d> <e> <f> .\n');
    });
    ...
    it('calls the done callback when ending the outputstream errors', done => {
      ...
      groupedWriter.end(done);
    });

    it('sends output through end when no stream argument is given', done => {
      const groupedWriter = new GroupedWriter();
      let notCalled = true;
      groupedWriter.addQuadGroup(new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c')), () => { notCalled = false; });
      groupedWriter.end((error, output) => {
    ...
    it(
      'respects the prefixes argument when no stream argument is given',
        ...
        const groupedWriter = new GroupedWriter({ prefixes: { a: 'b#' } });
        groupedWriter.addQuadGroup(new Quad(new NamedNode('b#a'), new NamedNode('b#b'), new NamedNode('b#c')));
        groupedWriter.end((error, output) => {
    ...
    it(
      'should serialize triples of graph with prefix for local names that begin with underscore',
        ...
        const groupedWriter = new GroupedWriter();
        GroupedWriter.addPrefix('a', 'b#');
        groupedWriter.addQuadGroup(new Quad(new NamedNode('b#_a'), new NamedNode('b#b'), new NamedNode('b#c'), new NamedNode('b#g')));
        groupedWriter.end((error, output) => {
          expect(output).toBe('@prefix a: <b#>.\n\na:g {\na:_a a:b a:c\n}\n');
          done(error);
        });
    ...
    it(
      'serializes triples of a graph with a prefix declaration in between',
        ...
        groupedWriter.addQuadGroup(new Quad(new NamedNode('b#a'), new NamedNode('b#b'), new NamedNode('b#c')));
        GroupedWriter.addPrefix('a', 'b#');
        groupedWriter.addQuadGroup(new Quad(new NamedNode('b#a'), new NamedNode('b#b'), new NamedNode('b#c'), new NamedNode('b#g')));
        GroupedWriter.addPrefix('d', 'e#');
        groupedWriter.addQuadGroup({ subject: new NamedNode('b#a'), predicate: new NamedNode('b#b'), object: new NamedNode('b#d'), graph: new NamedNode('b#g') });
        groupedWriter.end((error, output) => {
          expect(output).toBe('<b#a> <b#b> <b#c>.\n' +
                              '@prefix a: <b#>.\n\na:g {\na:a a:b a:c\n}\n' +
                              '@prefix d: <e#>.\n\na:g {\na:a a:b a:d\n}\n');
    ...
    it('should not write prefixes in N-Triples mode', done => {
      const groupedWriter = new GroupedWriter({ format: 'N-Triples', prefixes: { a: 'b#' } });
      let called = false;
      function callback() { called = true; }
      GroupedWriter.addPrefix('c', 'd#');
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new Literal('"c"'));
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new Literal(`"1"^^${xsd.integer}`));
      GroupedWriter.addPrefix('e', 'f#', callback);
      groupedWriter.end((error, output) => {
    ...
    it('uses a base IRI when given', done => {
      const groupedWriter = new GroupedWriter({ baseIRI: 'http://example.org/foo/' });
      groupedWriter.addQuadGroup(new Quad(
    ...
    it('does not use partially match base IRIs', done => {
      const groupedWriter = new GroupedWriter({ baseIRI: 'https://pod.example/profile/card' });
      groupedWriter.addQuadGroup(new Quad(
    ...
    it('should accept triples with separated components', done => {
      const groupedWriter = new GroupedWriter();
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new NamedNode('c'));
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new NamedNode('d'));
      groupedWriter.end((error, output) => {
    ...
    it('should accept quads with separated components', done => {
      const groupedWriter = new GroupedWriter();
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new NamedNode('c'), new NamedNode('g'));
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new NamedNode('d'), new NamedNode('g'));
      groupedWriter.end((error, output) => {
    ...
    it('should serialize triples with an empty blank node as object', done => {
      const groupedWriter = new GroupedWriter();
      groupedWriter.addQuadGroup(new NamedNode('a1'), new NamedNode('b'), GroupedWriter.blank());
      groupedWriter.addQuadGroup(new NamedNode('a2'), new NamedNode('b'), GroupedWriter.blank([]));
      groupedWriter.end((error, output) => {
    ...
    it('should serialize triples with the same blank node as object', done => {
      const groupedWriter = new GroupedWriter();
      const blank = GroupedWriter.blank();
      groupedWriter.addQuadGroup(blank, new NamedNode('a'), new NamedNode('b'));
      groupedWriter.addQuadGroup(blank, new NamedNode('c'), new NamedNode('d'));
      groupedWriter.end((error, output) => {
    ...
    it(
      'should serialize triples with a one-triple blank node as object',
        ...
        const groupedWriter = new GroupedWriter();
        groupedWriter.addQuadGroup(new NamedNode('a1'), new NamedNode('b'), GroupedWriter.blank(new NamedNode('d'), new NamedNode('e')));
        groupedWriter.addQuadGroup(new NamedNode('a2'), new NamedNode('b'), GroupedWriter.blank({ predicate: new NamedNode('d'), object: new NamedNode('e') }));
        groupedWriter.addQuadGroup(new NamedNode('a3'), new NamedNode('b'), GroupedWriter.blank([{ predicate: new NamedNode('d'), object: new NamedNode('e') }]));
        groupedWriter.end((error, output) => {
    ...
    it(
      'should serialize triples with a two-triple blank node as object',
        ...
        const groupedWriter = new GroupedWriter();
        groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), GroupedWriter.blank([
    ...
    it('should accept triples in bulk', done => {
      const groupedWriter = new GroupedWriter();
      groupedWriter.addQuadGroupGroup([new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c')),
    ...
    it('should not allow writing after end', done => {
      const groupedWriter = new GroupedWriter();
      groupedWriter.addQuadGroup(new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c')));
      groupedWriter.end();
      groupedWriter.addQuadGroup(new Quad(new NamedNode('d'), new NamedNode('e'), new NamedNode('f')), error => {
        expect(error).toBeInstanceOf(Error);
        expect(error).toHaveProperty('message', 'Cannot write because the GroupedWriter has been closed.');
        done();
      });
    });

    it('should write simple triples in N-Quads mode', done => {
      const groupedWriter = new GroupedWriter({ format: 'N-Quads' });
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new NamedNode('c'));
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new NamedNode('d'));
      groupedWriter.end((error, output) => {
        expect(output).toBe('<a> <b> <c> .\n<a> <b> <d> .\n');
    ...
    it('should write simple quads in N-Quads mode', done => {
      ...
      let called = false;
      function callback() { called = true; }
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new NamedNode('c'), callback);
      groupedWriter.addQuadGroup(new NamedNode('a'), new NamedNode('b'), new NamedNode('d'), new NamedNode('g'));
      groupedWriter.end((error, output) => {
        expect(called).toBe(true);
        expect(output).toBe('<a> <b> <c> .\n<a> <b> <d> <g> .\n');
    ...
    it('should end when the end option is not set', done => {
      const outputStream = new QuickStream(), groupedWriter = new GroupedWriter(outputStream, {});
    ...
    it('should end when the end option is set to true', done => {
      const outputStream = new QuickStream(), groupedWriter = new GroupedWriter(outputStream, { end: true });
      expect(outputStream).toHaveProperty('ended', false);
      groupedWriter.end(() => {
        expect(outputStream).toHaveProperty('ended', true);
        done();
    ...
    it('should not end when the end option is set to false', done => {
      const outputStream = new QuickStream(), groupedWriter = new GroupedWriter(outputStream, { end: false });
      expect(outputStream).toHaveProperty('ended', false);
      groupedWriter.end(() => {
        expect(outputStream).toHaveProperty('ended', false);
        done();
    ...
    it(
      ...
      () => {
        const groupedWriter = new GroupedWriter();
        expect(
          groupedWriter.quadToString(new Quad(new BlankNode('b1'), new NamedNode('b'), new Literal('l1')), new NamedNode('b'), new NamedNode('c')),
        ).toBe('<<_:b1 <b> "l">> <b> <c> .\n');
      },
    );
    ...
    it('should serialize a triple with a triple with iris as subject', () => {
      const groupedWriter = new GroupedWriter();
      expect(
        groupedWriter.quadToString(new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c')), new NamedNode('b'), new NamedNode('c')),
      ).toBe('<<<a> <b> <c>>> <b> <c> .\n');
    });
    ...
    it(
      ...
      () => {
        const groupedWriter = new GroupedWriter();
        expect(
          groupedWriter.quadToString(new Quad(new BlankNode('b1'), new BlankNode('b2'), new BlankNode('b3')), new NamedNode('b'), new NamedNode('c')),
    ...
    it('should serialize a triple with a triple as object', () => {
      const groupedWriter = new GroupedWriter();
      expect(
        groupedWriter.quadToString(new NamedNode('a'), new NamedNode('b'), new Quad(new BlankNode('b1'), new NamedNode('b'), new Literal('l1'))),
      ).toBe('<a> <b> <<_:b1 <b> "l">> .\n');
    });
    ...
    it('should serialize a triple with a triple with iris as object', () => {
      const groupedWriter = new GroupedWriter();
      expect(
        groupedWriter.quadToString(new NamedNode('a'), new NamedNode('b'), new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c'))),
      ).toBe('<a> <b> <<<a> <b> <c>>> .\n');
    });
    ...
    it(
      ...
      () => {
        const groupedWriter = new GroupedWriter();
        expect(
          groupedWriter.quadToString(new NamedNode('a'), new NamedNode('b'), new Quad(new BlankNode('b1'), new BlankNode('b2'), new BlankNode('b3'))),
    ...
    it(
      ...
      () => {
        const groupedWriter = new GroupedWriter();
        expect(
          groupedWriter.quadToString(new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c')), new NamedNode('b'), new NamedNode('c'), new NamedNode('g')),
        ).toBe('<<<a> <b> <c>>> <b> <c> <g> .\n');
      },
    );
    ...
    it('should serialize a quad with a triple as object', () => {
      const groupedWriter = new GroupedWriter();
      expect(
        groupedWriter.quadToString(new NamedNode('a'), new NamedNode('b'), new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c')), new NamedNode('g')),
      ).toBe('<a> <b> <<<a> <b> <c>>> <g> .\n');
    });
    ...
    it('should serialize a quad with a quad as subject', () => {
      const groupedWriter = new GroupedWriter();
      expect(
        groupedWriter.quadToString(new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c'), new NamedNode('g')), new NamedNode('b'), new NamedNode('c'), new NamedNode('g')),
      ).toBe('<<<a> <b> <c> <g>>> <b> <c> <g> .\n');
    });
    ...
    it('should serialize a quad with a quad as object', () => {
      const groupedWriter = new GroupedWriter();
      expect(
        groupedWriter.quadToString(new NamedNode('a'), new NamedNode('b'), new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c'), new NamedNode('g')), new NamedNode('g')),
      ).toBe('<a> <b> <<<a> <b> <c> <g>>> <g> .\n');
    });
    ...
    it('should serialize a triple with a quad as subject', () => {
      const groupedWriter = new GroupedWriter();
      expect(
        groupedWriter.quadToString(new Quad(new NamedNode('a'), new NamedNode('b'), new NamedNode('c'), new NamedNode('g')), new NamedNode('b'), new NamedNode('c')),
      ).toBe('<<<a> <b> <c> <g>>> <b> <c> .\n');
    });
  ...
  return function (done) {
    ...
    (function next() {
      ...
      if (item) {
        ...
        groupedWriter.addQuadGroup(new Quad(subject, predicate, object, graph), next);
      }
      else
